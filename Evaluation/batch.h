...
#pragma omp critical
		...
		//Store original folds
		auto foldPath = aDataPath + "/originalFolds/Fold-" + QString::number( i + 1 );
		dir.mkpath( foldPath );
		saveFoldAt( foldPath, *trainingDataPair, *validationDataPair );

		//Store preprocessed folds		
		foldPath = aDataPath + "/preprocessedFolds/Fold-" + QString::number( i + 1 );
		dir.mkpath( foldPath );
...
void multipleCenterAnalysis( const QString& aGlobalSettingsPath, const QString& aDataPath )
{
	...
	lpmldata::DataPackage trainingData( trainingFDB, trainingLDB );
	lpmldata::DataPackage validationData( validationFDB, validationLDB );
	...
	dkeval::CentralAi ai( &settings, &pluginSettings, trainingData, validationData );
	ai.setFoldId( 1 );
	ai.execute();
	...
	//Store original folds		
	auto foldPath = aDataPath + "/originalFolds/Fold-" + QString::number( 1 );
	dir.mkpath( foldPath );
	saveFoldAt( foldPath, trainingData, validationData );

	//Store preprocessed folds
	foldPath = aDataPath + "/preprocessedFolds/Fold-" + QString::number( 1 );
	dir.mkpath( foldPath );
	ai.saveFoldAt( foldPath );
...
/*!
* \brief Runs the single/multi center automated data preparation execution
